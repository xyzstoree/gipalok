...
if ! jq -r '
  ...
  | select(.tag=="vmess-ws" or .tag=="vmess-grpc")
  | .settings.clients[]?.email // empty
' "$CONFIG" | grep -qx "$user"; then
    echo "Username tidak ditemukan di config: $user"
    sleep 2
    back_menu
    exit 1
fi
...
for job in $(atq 2>/dev/null | awk '{print $1}'); do
    at -c "$job" 2>/dev/null | grep -q "/usr/local/bin/clean-vmess-${user}.sh" && atrm "$job" 2>/dev/null || true
done

rm -f "/usr/local/bin/clean-vmess-${user}.sh"
rm -f /etc/Anggun/Queue/vmess/ip/"$user" /etc/Anggun/Queue/vmess/quota/"$user"
rm -f /etc/Anggun/Expiry/vmess/"$user"
rm -f /etc/Anggun/Usage/vmess/"$user"
rm -f /etc/Anggun/vmess/"$user" /etc/Anggun/cache/vmess-ws/"$user"

systemctl restart xray >/dev/null 2>&1 || true

clear
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "Delete VMess Berhasil"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "Username : $user"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
