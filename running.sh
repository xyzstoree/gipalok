...
# Author  : Anggun / GIPALOK
# =========================================

export LC_ALL='en_US.UTF-8'
export LANG='en_US.UTF-8'
export LANGUAGE='en_US.UTF-8'
# FIX: LC_CTYPE was 'en_US.utf8' (wrong case) — corrected to 'en_US.UTF-8'
...
# FIX: Author was 'FsidVPN' — corrected to 'GIPALOK'
export Author="GIPALOK"
export RED_BG='\e[41m'

export IP=$(curl -s --max-time 5 https://ipinfo.io/ip/ 2>/dev/null || echo "N/A")

# Helper stubs (previously called but undefined — caused error output)
os_detail()      { :; }
script_version() { :; }
license_check()  { :; }
...
# FIX: SSH NonTLS and SSH TLS were BOTH checking ws-stunnel → showed same status.
#      NonTLS WS uses 'ws-epro' service; TLS WS uses 'ws-stunnel' service.
...
os_detail; script_version; license_check
...
# FIX: Redundant clear calls removed (was called 7 times — caused flicker & slowdown)
clear
...
# FIX: NonTLS and TLS now show their own independent status
