...
def clean_username(user):
    ...
    if not re.match(r"^[a-zA-Z0-9_][a-zA-Z0-9_-]{1,31}$", user):
        raise ValueError("Username hanya boleh huruf, angka, underscore, dash, panjang 2-32 karakter")
    return user
...
def save_config(cfg):
    ...
    test = sh(f"/usr/local/bin/xray test -config {tmp}")
...
def ensure_metadata(cfg, proto, user, meta):
    if "_metadata" not in cfg or not isinstance(cfg["_metadata"], dict):
        cfg["_metadata"] = {}

    if proto not in cfg["_metadata"] or not isinstance(cfg["_metadata"][proto], dict):
        cfg["_metadata"][proto] = {}

    cfg["_metadata"][proto][user] = meta
...
def create_ssh(params):
    ...
    exists = sh(f"id {user}").returncode == 0
...
def create_xray(proto, params):
    ...
    if proto == "vmess":
        ...
        tls = f"vless://{uid}@{domain}:443?security=tls&encryption=none&type=ws&host={domain}&path=%2Fvless&sni={domain}#{user}"
        ntls = f"vless://{uid}@{domain}:80?security=none&encryption=none&type=ws&host={domain}&path=%2Fvless#{user}"
        data.update(
            {
                "vless_tls_link": tls,
                "vless_nontls_link": ntls,
                "link_tls": tls,
                "link_nontls": ntls,
            }
        )
        ...
        tls = f"trojan://{uid}@{domain}:443?security=tls&type=ws&host={domain}&path=%2Ftrojan-ws&sni={domain}#{user}"
        ntls = f"trojan://{uid}@{domain}:80?security=none&type=ws&host={domain}&path=%2Ftrojan-ws#{user}"
        data.update(
            {
                "trojan_tls_link": tls,
                "trojan_nontls_link": ntls,
                "link_tls": tls,
                "link_nontls": ntls,
            }
        )
        ...
        try:
            ...
            if path in ["", "health", "status"]:
                return response(
                    ...
                    {
                        "status": "success",
                        "message": "BotVPN API active",
                        "port": PORT,
                        "api_type": 2,
                    },
                )

            if not auth_ok(params, self.headers):
                return response(
                    self,
                    401,
                    {
                        "status": "error",
                        "message": "Unauthorized / auth token salah",
                    },
                )
...
def main():
    ...
    print(f"BotVPN API running on {HOST}:{PORT}")
